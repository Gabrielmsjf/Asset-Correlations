...
DOL_Index <- read.csv( "USDIndDec19_Daily.csv")
...
#############
DOL_Index$DATE <- DOL_Index$DATE %>%
  as.character() %>%
  gsub(pattern = ".", replacement = "", fixed = TRUE)

DOL_Index_DATE <- as.POSIXct(DOL_Index$DATE, format = "%Y %m %d") 

DOL_Index <- as.xts(DOL_Index[ ,2:5], order.by = DOL_Index_DATE)
#####################################################################################################################
################################# Get all Retuns ######################################################################

DAX_rets <- na.omit(Return.calculate(DAX[,4]))
DOL_Index_rets <- na.omit(Return.calculate(DOL_Index[,4]))
...
Market_df <- na.omit(merge(DAX_rets, DOL_Index_rets, Dow_Jones_rets, Gold_rets, IBOV_rets, OIL_rets, US_TB_rets, US_Vix_rets,
             USD_BRL_rets, USD_Index_rets))

missmap(Market_df)

names(Market_df) <- c("DAX", "DOL_Index", "Dow_Jones", "Gold", "IBOV", "OIL", "US_TB", "US_Vix", "USD_BRL", "USD_Index")

chart.Correlation(Market_df)
chart.CumReturns(Market_df, legend.loc = "topleft", main = "Correlaçao Ativos")  
